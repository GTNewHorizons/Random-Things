...
public class RTConfiguration
{
	...
	{
		System.out.println(event.configID);
	...
	public static void syncConfig()
	{
		...
		VanillaChanges.MODIFIED_BACKGROUND = RTConfiguration.config.get("VanillaChanges", "ModifiedBackgrounds", true, "The normal dirt background will be replaced with a different block each start").getBoolean(true);
		VanillaChanges.LOCKED_GAMMA = RTConfiguration.config.get("VanillaChanges", "LockedGamma", false, "Locks the Gamma to 0").getBoolean(false);
		...
		BackgroundHandler.fixedBackground = RTConfiguration.config.get("VanillaChanges", "fixedBackground", "", "If this is not empty the options background will not be random but the one specified here. This has to be the name of a block texture without the .png").getString();
		...
		Settings.ANIMATED_TEXTURES = RTConfiguration.config.get("Settings", "AnimatedTextures", true).getBoolean(true);
		...
		Settings.SPECTRE_DIMENSON_ID = RTConfiguration.config.get("Settings", "SpectreDimensionID", -1, "The Dimension ID of the spectre World. On first run and when you remove this setting from the config file RandomThings will try to find a dimensionID itself").getInt();
