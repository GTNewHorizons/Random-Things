...
public class BlockAdvancedFluidDisplay extends BlockContainerBase
{
	...
	{
		TileEntityAdvancedFluidDisplay te = (TileEntityAdvancedFluidDisplay) par1IBlockAccess.getTileEntity(posX, posY, posZ);
		if (te == null || te.getFluid(side) == null || te.getFluid(side).equals("") || FluidRegistry.getFluid(te.getFluid(side)) == null)
		{
			return blockIcon;
		}
		else
	...
	@Override
	public boolean onBlockActivated(World world, int posX, int posY, int posZ, EntityPlayer entityplayer, int side, float par7, float par8, float par9)
	{
