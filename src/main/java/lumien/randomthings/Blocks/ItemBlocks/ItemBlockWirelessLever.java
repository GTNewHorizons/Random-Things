            ...
            if (player.getCurrentEquippedItem() == par1ItemStack && player.isSneaking()) {
                if (par1ItemStack.stackTagCompound != null && par1ItemStack.stackTagCompound.getBoolean("hasTarget")) {
                    ...
                    for (double d = 0; d <= 1; d += 0.02d) {
                        EntityReddustFX particle = new EntityReddustFX(
                                worldObj,
                                xCoord + 0.5 + vec.x * d,
                                yCoord + 0.5 + vec.y * d,
                                zCoord + 0.5 + vec.z * d,
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (stack.stackTagCompound == null || !stack.stackTagCompound.getBoolean("hasTarget")) {
            ...
            if (placed) {
                ...
                te.setTarget(
                        stack.stackTagCompound.getInteger("targetX"),
                        stack.stackTagCompound.getInteger("targetY"),
                        stack.stackTagCompound.getInteger("targetZ"));
            }
            return placed;
        }
    }
