...
public class BlockPattern
{
	...
	public boolean matches(World worldObj, int posX, int posY, int posZ)
	{
		for (BlockInfo info : blockInfos)
		{
			if (worldObj.getBlock(posX + info.modX, posY + info.modY, posZ + info.modZ) != info.block || worldObj.getBlockMetadata(posX + info.modX, posY + info.modY, posZ + info.modZ) != info.metadata)
			{
				return false;
			}
		}
