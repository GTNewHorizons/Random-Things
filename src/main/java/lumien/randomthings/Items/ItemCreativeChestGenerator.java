...
public class ItemCreativeChestGenerator extends ItemCreative
{
	...
	@Override
	public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer entityPlayer, World worldObj, int posX, int posY, int posZ, int side, float par8, float par9, float par10)
	{
			...
			else
			{
				if (worldObj.isAirBlock(posX, posY, posZ))
				{
					if (Blocks.chest.canPlaceBlockAt(worldObj, posX, posY, posZ))
					{
						...
						ChestCategory category = ChestCategory.values()[par1ItemStack.stackTagCompound.getInteger("category")];
						worldObj.setBlock(posX, posY, posZ, Blocks.chest);
						WeightedRandomChestContent.generateChestContents(rng, ChestGenHooks.getItems(category.getName(), rng), (IInventory) worldObj.getTileEntity(posX, posY, posZ), ChestGenHooks.getCount(category.getName(), rng));
					}
				}
				return true;
			}
